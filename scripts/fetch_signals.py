...
# Metrik fundamental untuk panel "Fundamental & Risiko". Semuanya berasal
# dari .info yang SUDAH ditarik untuk skoring — jadi tidak ada satu pun
# request jaringan tambahan. Sebuah build_fundamentals.py terpisah akan
# mengulangi kesalahan yang sama seperti build_indicators.py: melipatduakan
# lintasan jaringan 30-60 menit demi data yang sudah ada di tangan.
#
# Riwayat earnings surprise sengaja TIDAK diambil: Ticker.earnings_dates
# adalah request kedua per ticker (+984 request), sementara .info sudah
# memuat earningsGrowth dan earningsQuarterlyGrowth.
FUNDAMENTAL_FIELDS = {
    # profitabilitas
    "roe": ("returnOnEquity", 4),
    "roa": ("returnOnAssets", 4),
    "grossMargin": ("grossMargins", 4),
    "operatingMargin": ("operatingMargins", 4),
    "profitMargin": ("profitMargins", 4),
    # valuasi
    "trailingPE": ("trailingPE", 2),
    "forwardPE": ("forwardPE", 2),
    "priceToBook": ("priceToBook", 2),
    "priceToSales": ("priceToSalesTrailing12Months", 2),
    "evEbitda": ("enterpriseToEbitda", 2),
    "peg": ("trailingPegRatio", 2),
    # pertumbuhan
    "revenueGrowth": ("revenueGrowth", 4),
    "earningsGrowth": ("earningsGrowth", 4),
    "earningsQGrowth": ("earningsQuarterlyGrowth", 4),
    # neraca
    "currentRatio": ("currentRatio", 2),
    "quickRatio": ("quickRatio", 2),
    "debtToEquity": ("debtToEquity", 1),
    # kepemilikan & sentimen posisi
    "shortPctFloat": ("shortPercentOfFloat", 4),
    "heldInstitutions": ("heldPercentInstitutions", 4),
    "heldInsiders": ("heldPercentInsiders", 4),
}


def fundamentals_block(info: dict) -> dict:
    """Metrik fundamental terpakai dari .info, dibulatkan, tanpa entri kosong."""
    out = {}
    for name, (key, decimals) in FUNDAMENTAL_FIELDS.items():
        value = info.get(key)
        if value is None:
            continue
        try:
            out[name] = round(float(value), decimals)
        except (TypeError, ValueError):
            continue

    # FCF yield tidak disediakan langsung; ia yang paling sulit dipoles
    # akuntansi, jadi layak dihitung sendiri.
    fcf, mcap = info.get("freeCashflow"), info.get("marketCap")
    if fcf is not None and mcap:
        out["fcfYield"] = round(fcf / mcap, 4)
    return out


def write_fundamentals(updates: dict) -> None:
    """Tulis data/fundamentals.js, mempertahankan ticker yang tak diproses."""
    if not updates:
        return
    existing: dict = {}
    if FUNDAMENTALS_JS.exists():
        try:
            raw = FUNDAMENTALS_JS.read_text(encoding="utf-8")
            m = re.search(r"window\.STOCK_FUNDAMENTALS\s*=\s*(\{.*\})\s*;", raw, re.DOTALL)
            if m:
                existing = json.loads(m.group(1))
        except (UnicodeDecodeError, json.JSONDecodeError, OSError):
            existing = {}
    existing.update(updates)
    FUNDAMENTALS_JS.write_text(
        "// Auto-generated by scripts/fetch_signals.py — jangan edit manual.\n"
        "// Di-load LAZY oleh dashboard, bukan saat muat awal.\n"
        "window.STOCK_FUNDAMENTALS = "
        + json.dumps(existing, separators=(",", ":"), sort_keys=True) + ";\n",
        encoding="utf-8",
    )
    print(f"Fundamental ditulis ke {FUNDAMENTALS_JS} "
          f"({len(updates)} diperbarui, {len(existing)} total)")
...
def fetch(ticker: str, bench_hist: "pd.DataFrame") -> Optional[dict]:
    ...
    try:
        ...
        payload = {
            ...
            "_fundamentals": fundamentals_block(info),
        }
